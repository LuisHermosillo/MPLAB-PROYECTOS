...
void __attribute__((interrupt, no_auto_psv)) _CNInterrupt( void){
    ...
    if(PORTBbits.RB8 == 0){
       ...
       if(PORTBbits.RB3 == 0){numeroDecimal = 13; band = 1; opcion = 0; cambio = 'X';}//D
    }
    LATBbits.LATB8 = 1;
    if(PORTBbits.RB6 == 0){
    ...
    if(band == 1){
        ...
        putsLCD("                     "); // Escribe "0" la pantalla (renglon 2)
        setDDRamAddr(0x0);
        putsLCD("       Datos         "); // Escribe "CONTADOR" en el primer 
        if(opcion == 0){
            if(numeroDecimal < 10){
                if(contadorDigitos <= 8){
                    if(cambio == 'X'){
                        setDDRamAddr(0x0);
                        putsLCD("X   dato      "); // Escribe "CONTADOR" en el primer 
                    ...
                    if(cambio == 'H'){
                        setDDRamAddr(0x0);
                        putsLCD("H   dato      "); // Escribe "CONTADOR" en el primer 
                        Digitos[contadorDigitos] = numeroDecimal;
                        valor2 = Numero();
                        sprintf(cadena, "%.0f", valor2); // Convierte el valor 	en contador a cadena en cad_contador
                        setDDRamAddr(0x40); // Se mueve al renglon 2
                        putsLCD(cadena); // Escribe el valor del contador
                        contadorDigitos++;
                        cambio = 'H';
            ...
            if(cambio == 'X'){
                if(numeroDecimal == 14){ //Guarda como negativo
                    x_datos[count_x] = -(valor1);
                    contadorDigitos = 0;
                    count_x++;
                    valor1 = 0;
                }
                if(numeroDecimal == 12){ //Guarda como positivo
                    x_datos[count_x] = valor1;
                    contadorDigitos = 0;
                    count_x++;
                    valor1 = 0;
            ...
            if(numeroDecimal == 11){
                ...
                int k;
                for (k = 0; k < count_h; k++) {
                    sprintf(cadena, "%.0f", h_datos[k]); // Convierte el valor 	en contador a cadena en cad_contador
                    setDDRamAddr(0x40); // Se mueve al renglon 2
                    putsLCD(cadena); // Escribe el valor del contador
                    __delay_ms(1000);
                    setDDRamAddr(0x40);
